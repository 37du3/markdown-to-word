...
describe('DocxConverter', () => {
  ...
  it('should include tables in document children', () => {
    const markdown = '| A | B |\n| --- | --- |\n| 1 | 2 |';
    const ast = parser.parse(markdown);
    const children = (converter as unknown as {
      buildDocumentChildren: (input: typeof ast, opts: ConversionOptions) => unknown[];
    }).buildDocumentChildren(ast, options);

    expect(children.some((child) => child instanceof Table)).toBe(true);
  });

  it('should strip inline markdown in table cells', () => {
    const markdown = '| **A** | B |\n| --- | --- |\n| 1 | **2** |';
    const ast = parser.parse(markdown);
    const children = (converter as unknown as {
      buildDocumentChildren: (input: typeof ast, opts: ConversionOptions) => unknown[];
    }).buildDocumentChildren(ast, options);
    const table = children.find((child) => child instanceof Table) as Table;

    const text = extractTableText(table).join(' ');
    expect(text).toContain('A');
    expect(text).toContain('2');
    expect(text).not.toContain('**');
  });

  it('should apply rowspan and colspan for merged cells', () => {
    ...
    const children = (converter as unknown as {
      buildDocumentChildren: (input: typeof ast, opts: ConversionOptions) => unknown[];
    }).buildDocumentChildren(ast, mergeOptions);
    const table = children.find((child) => child instanceof Table) as Table;

    const props = findCellProps(table, 1, 0);
...
function extractTableText(table: Table): string[] {
  const rows = (table.root || []).filter((node: any) => node.rootKey === 'w:tr');
  const texts: string[] = [];
